...
exports.HIDDEN_STEMS = exports.STEM_TEN_GOD_MAP = exports.BRANCH_TEN_GOD_MAP = void 0;
exports.getBranchTenGodFromMap = getBranchTenGodFromMap;
exports.getStemTenGodFromMap = getStemTenGodFromMap;
 ...
 * @returns 十神関係を表す文字列
 */
function getBranchTenGodFromMap(dayStem, branch) {
    var stemIndex = getStemIndex(dayStem);
    var branchIndex = getBranchIndex(branch);
    if (stemIndex === -1 || branchIndex === -1) {
        return "不明";
    }
    return exports.BRANCH_TEN_GOD_MAP[stemIndex][branchIndex];
}
/**
 * 蔵干の十神関係マッピング
 * 天干×天干の組み合わせによる十神関係テーブル
 */
exports.STEM_TEN_GOD_MAP = [
    // 甲日の十神（行）
 ...
 * @returns 十神関係を表す文字列
 */
function getStemTenGodFromMap(dayStem, targetStem) {
    var dayStemIndex = getStemIndex(dayStem);
    var targetStemIndex = getStemIndex(targetStem);
    if (dayStemIndex === -1 || targetStemIndex === -1) {
        return "不明";
    }
    return exports.STEM_TEN_GOD_MAP[dayStemIndex][targetStemIndex];
}
/**
 * 地支の蔵干情報
 * 各地支に含まれる天干のリスト
...
function determineBranchTenGodRelation(dayStem, branch) {
    // 地支と日主から十神関係を取得
    var mainTenGod = getBranchTenGodFromMap(dayStem, branch);
    // 地支の蔵干を取得
    var hiddenStems = exports.HIDDEN_STEMS[branch] || [];
    // 蔵干ごとの十神関係を計算
    var hiddenTenGods = hiddenStems.map(function (stem) { return ({
        stem: stem,
