...
const WEALTH_RELATIONSHIPS: Record<string, string[]> = {
  // 木の地支に対する金は財
  '寅': ['申', '酉'], '卯': ['申', '酉'],
  // 火の地支に対する土は財
  '巳': ['辰', '丑', '戌', '未'], '午': ['辰', '丑', '戌', '未'],
 ...
 * @returns 年殺かどうか
 */
export function isYearSpirit(yearBranch: string, dayBranch: string): boolean {
 ...
 * @returns 月殺かどうか
 */
export function isMonthSpirit(monthBranch: string, dayBranch: string): boolean {
 ...
 * @returns 日殺かどうか
 */
export function isDaySpirit(dayBranch: string, hourBranch: string): boolean {
 ...
 * @returns 六害殺かどうか
 */
export function isSixHarmSpirit(
  yearBranch: string,
  monthBranch: string,
  dayBranch: string,
  hourBranch: string
  ...
  // 年柱と他の柱の間の六害関係
  if (SIX_HARMS[yearBranch] === monthBranch) harmCount++;
  if (SIX_HARMS[yearBranch] === dayBranch) harmCount++;
  if (SIX_HARMS[yearBranch] === hourBranch) harmCount++;
  
  // 月柱と他の柱の間の六害関係
  if (SIX_HARMS[monthBranch] === yearBranch) harmCount++;
  if (SIX_HARMS[monthBranch] === dayBranch) harmCount++;
  if (SIX_HARMS[monthBranch] === hourBranch) harmCount++;
  
  // 日柱と他の柱の間の六害関係
  if (SIX_HARMS[dayBranch] === yearBranch) harmCount++;
  if (SIX_HARMS[dayBranch] === monthBranch) harmCount++;
  if (SIX_HARMS[dayBranch] === hourBranch) harmCount++;
  
  // 時柱と他の柱の間の六害関係
  if (SIX_HARMS[hourBranch] === yearBranch) harmCount++;
  if (SIX_HARMS[hourBranch] === monthBranch) harmCount++;
  if (SIX_HARMS[hourBranch] === dayBranch) harmCount++;
 ...
 * @returns 財殺かどうか
 */
export function isWealthSpirit(
  yearBranch: string,
  monthBranch: string,
  dayBranch: string,
  hourBranch: string,
  ...
  const hasWoodBranch = [yearBranch, monthBranch, dayBranch, hourBranch].some(
    branch => branch === '寅' || branch === '卯'
  );
  
  const hasMetalBranch = [yearBranch, monthBranch, dayBranch, hourBranch].some(
    branch => branch === '申' || branch === '酉'
  );
 ...
 * @returns 劫殺かどうか
 */
export function isRobberySpirit(
  yearBranch: string, 
  monthBranch: string, 
  dayBranch: string, 
  hourBranch: string,
  ...
  // 2. 特定の組み合わせパターン
  const hasTiger = yearBranch === '寅' || monthBranch === '寅' || dayBranch === '寅' || hourBranch === '寅';
  const hasMonkey = yearBranch === '申' || monthBranch === '申' || dayBranch === '申' || hourBranch === '申';
...
export function isHourSpirit(
  ...
  // 拡張: 日柱と時柱の相互作用
  if (
    (dayStem === '乙' && hourStem === '丁' && hourBranch === '巳') ||
  ...
  if (
    (hourBranch === '子' && (dayStem === '戊' || dayStem === '己')) ||
 ...
 * @returns 逆馬殺かどうか
 */
export function isReversedHorseSpirit(
  yearStem: string,
  monthStem: string,
  dayStem: string,
  hourStem: string,
  ...
  const stemElementConflicts: Record<string, string[]> = {
    '甲': ['申', '酉'], // 木の天干に対する金の地支
    '乙': ['申', '酉'],
    '丙': ['亥', '子'], // 火の天干に対する水の地支
    '丁': ['亥', '子'],
...
export function calculateTwelveSpirits(
  ...
  // 1. 基本的な殺の判定（年殺、月殺、日殺）
  const hasYearSpirit = isYearSpirit(yearBranch, dayBranch);
  const hasMonthSpirit = isMonthSpirit(monthBranch, dayBranch);
  const hasDaySpirit = isDaySpirit(dayBranch, hourBranch);
  
  // 2. 複合的な殺の判定
  const hasSixHarmSpirit = isSixHarmSpirit(yearBranch, monthBranch, dayBranch, hourBranch);
  const hasWealthSpirit = isWealthSpirit(yearBranch, monthBranch, dayBranch, hourBranch, yearStem, monthStem, dayStem, hourStem);
  const hasHopeSpirit = isHopeSpirit(yearBranch, monthBranch, dayBranch, hourBranch, yearStem, monthStem, dayStem, hourStem);
  const hasRobberySpirit = isRobberySpirit(yearBranch, monthBranch, dayBranch, hourBranch, yearStem, monthStem, dayStem, hourStem);
  const hasHourSpirit = isHourSpirit(yearBranch, monthBranch, dayBranch, hourBranch, yearStem, monthStem, dayStem, hourStem);
  const hasReversedHorseSpirit = isReversedHorseSpirit(yearStem, monthStem, dayStem, hourStem, yearBranch, monthBranch, dayBranch, hourBranch);
 ...
 * 十二神殺のテスト関数
 */
export function testTwelveSpiritCalculator(): void {
  console.log('--- 十二神殺計算テスト ---');
  
  // サンプル1: 年殺のテスト
  const test1 = calculateTwelveSpirits(
    '甲', '丙', '壬', '戊',
    '子', '子', '辰', '子'
  );
  ...
  // サンプル2: 月殺のテスト
  const test2 = calculateTwelveSpirits(
    '癸', '癸', '壬', '庚',
    '卯', '丑', '辰', '子'
  );
  ...
  const test3 = calculateTwelveSpirits(
    '壬', '癸', '癸', '壬',
    '寅', '丑', '巳', '子'
  );
  ...
  const test4 = calculateTwelveSpirits(
    '癸', '辛', '丙', '戊',
    '卯', '酉', '申', '子'
  );
  ...
  const test5 = calculateTwelveSpirits(
    '癸', '壬', '丙', '丙',
    '卯', '戌', '午', '申'
  );
  ...
  // サンプル6: 望神殺テスト - 2005年
  const test6 = calculateTwelveSpirits(
    '甲', '丙', '乙', '丙',
    '申', '子', '酉', '子'
  );
  ...
  const test7 = calculateTwelveSpirits(
    '癸', '乙', '戊', '壬',
    '亥', '丑', '辰', '子'
  );
  ...
  const test8 = calculateTwelveSpirits(
    '癸', '壬', '丙', '庚',
    '卯', '戌', '午', '寅'
  );
  ...
  // サンプル9: 六害殺テスト
  const test9 = calculateTwelveSpirits(
    '壬', '辛', '戊', '壬',
    '申', '酉', '辰', '子'
  );
  ...
  // サンプル10: 財殺テスト
  const test10 = calculateTwelveSpirits(
    '丙', '戊', '甲', '戊',
    '午', '辰', '寅', '申'
  );
  ...
  // サンプル11: 時殺テスト
  const test11 = calculateTwelveSpirits(
    '癸', '壬', '丙', '甲',
    '卯', '戌', '午', '子'
  );
  ...
  // サンプル12: 逆馬殺テスト
  const test12 = calculateTwelveSpirits(
    '癸', '壬', '甲', '庚',
    '卯', '戌', '午', '申'
  );
  ...
  // サンプル13: 複数の殺が競合するケース
  const test13 = calculateTwelveSpirits(
    '丙', '庚', '甲', '壬',
    '午', '申', '子', '寅'
  );
 ...
 * 複数のサンプルデータを検証する関数
 */
export function validateWithSamples(): void {
  ...
  samples.forEach((sample, index) => {
    const result = calculateTwelveSpirits(
    ...
    console.log(`サンプル${index + 1} 計算結果:`, result);
    console.log(`サンプル${index + 1} 期待結果:`, sample.expected);
    
    // 一致しているか確認
    const yearMatch = result.year === sample.expected.year;
    const monthMatch = result.month === sample.expected.month;
    const dayMatch = result.day === sample.expected.day;
    const hourMatch = result.hour === sample.expected.hour;
    
    console.log(`サンプル${index + 1} 一致状況:`, {
      year: yearMatch ? '✓' : '✗',
      month: monthMatch ? '✓' : '✗',
      day: dayMatch ? '✓' : '✗',
      hour: hourMatch ? '✓' : '✗'
    ...
    // 総合的な一致率
    const totalMatches = [yearMatch, monthMatch, dayMatch, hourMatch].filter(Boolean).length;
    console.log(`サンプル${index + 1} 一致率: ${totalMatches / 4 * 100}%\n`);
  });
  ...
  samples.forEach((sample) => {
    ...
    // 各柱ごとに確認
    if (result.year === sample.expected.year) totalMatches++;
    if (result.month === sample.expected.month) totalMatches++;
    if (result.day === sample.expected.day) totalMatches++;
    if (result.hour === sample.expected.hour) totalMatches++;
    
    totalComparisons += 4; // 4柱分
  });
  
  console.log(`全体の一致率: ${totalMatches / totalComparisons * 100}%`);
}
