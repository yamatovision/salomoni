...
|-----------|--------------|--------|------|----------|----------------------|--------------|------------|-----------------| 
| **1.1** | `/api/auth/register-organization` | POST | 組織新規登録（Owner作成） | 不要 | 組織登録ページ | [ ] | [ ] | [ ] |
| **1.2** | `/api/auth/login` | POST | メール/パスワードログイン | 不要 | ログインページ | [ ] | [ ] | [ ] |
| **1.3** | `/api/auth/login-line` | POST | LINE認証ログイン | 不要 | ログインページ | [ ] | [ ] | [ ] |
| **1.4** | `/api/auth/refresh` | POST | トークンリフレッシュ | 必要 | 共通処理 | [ ] | [ ] | [ ] |
| **1.5** | `/api/auth/logout` | POST | ログアウト | 必要 | ヘッダーコンポーネント | [ ] | [ ] | [ ] |
| **1.6** | `/api/users/me` | GET | 現在のユーザー情報取得 | 必要 | 全ページ共通 | [ ] | [ ] | [ ] |
| **2.1** | `/api/users/invite` | POST | スタイリスト招待 | 必要 | スタイリスト管理ページ | [ ] | [ ] | [ ] |
| **2.2** | `/api/auth/complete-registration` | POST | 招待受諾・初回登録完了 | 不要 | 初回設定ページ | [ ] | [ ] | [ ] |
| **2.3** | `/api/admin/stylists` | GET | スタイリスト一覧取得 | 必要 | スタイリスト管理ページ | [ ] | [ ] | [ ] |
| **2.4** | `/api/admin/stylists/:id` | GET | スタイリスト詳細取得 | 必要 | スタイリスト管理ページ | [ ] | [ ] | [ ] |
| **2.5** | `/api/admin/stylists/:id` | PUT | スタイリスト情報更新 | 必要 | スタイリスト管理ページ | [ ] | [ ] | [ ] |
...
| **10.3** | `/api/organizations/:id/stats` | GET | 組織統計情報取得 | 必要 | 管理者ダッシュボード | [ ] | [ ] | [ ] |
...
| **7. 環境変数設定** | [ ] | ★7 環境変数収集アシスタント | OpenAI API・Apple認証・LINE認証・カレンダー連携の秘密鍵設定 |
| **8. バックエンド実装** | [ ] | ★8 バックエンド実装エージェント | マルチテナント対応・四柱推命計算・AIチャット統合実装 |
| **9. テスト品質検証** | [ ] | ★9 テスト品質エンジニア | 組織間データ分離・AI応答品質・負荷テスト |
| **10. フロントエンド実装** | [ ] | ★10 フロントエンド実装エージェント | React/React Native・3アプリケーション統合開発 |
...
- 📋 次のアクション：環境変数収集アシスタントを使って本番環境で動作するための秘密鍵を取得
